...
namespace App\Repositories\Mobile\Config;
...
class ConfigMobileDeveloperRepository implements ConfigMobileDeveloperRepositoryInterface
{
    use GeneralTrait;
...
//            });
            $languages=config('environment_system.available_languages');
    ...
    public function change_language($request)
    {
        ...
        try {
            ...
            if ($lang=='ar'){
                return $this->returnData('language',$lang,'تم تغيير اللغة بنجاح');
            }
            if ($lang=='tr'){
                return $this->returnData('language',$lang,'Dil başarıyla değiştirildi');
            }
            return $this->returnData('language',$lang,'The language has been changed successfully');
        }catch (\Exception $ex) {
            return $this->returnError($ex->getCode(), $ex->getMessage());
        }
    }
